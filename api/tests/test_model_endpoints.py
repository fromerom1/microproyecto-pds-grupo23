...
@pytest.fixture
def modelo(cohorte_sintetica) -> ModeloRiesgo:
    return ModeloRiesgo("24m", cohorte_path=cohorte_sintetica)
...
def test_predict_matches_model_with_defaults(cliente: TestClient, modelo: ModeloRiesgo) -> None:
    ...
    assert response.json() == {**modelo.predecir(registro), "variante": modelo.variante}
...
def test_predict_accepts_horizon_and_variant(
    cliente: TestClient, modelo: ModeloRiesgo, cohorte_sintetica
) -> None:
    ...
    expected_model = ModeloRiesgo(
        modelo.horizonte, alternativa, cohorte_path=cohorte_sintetica
    )
    registro = {
        campo: valor
    ...
    assert response.json() == {
        **expected_model.predecir(registro), "variante": expected_model.variante
    }


def test_predict_accepts_other_horizon(
    cliente: TestClient, modelo: ModeloRiesgo, cohorte_sintetica
) -> None:
    alternativo = next(
    ...
    expected_model = ModeloRiesgo(alternativo, cohorte_path=cohorte_sintetica)
    registro = {
        campo: valor
        for campo, valor in registro_demo(modelo).items()
        if campo in expected_model.features
...
@pytest.mark.parametrize("ruta", ["/predict", "/predict/batch"])
def test_rejects_text_in_numeric_field(
    cliente: TestClient, modelo: ModeloRiesgo, ruta: str
) -> None:
    registro = registro_demo(modelo)
    registro["gestage_final"] = "36"
    cuerpo = {"registros": [registro]} if ruta.endswith("batch") else {"registro": registro}
...
def test_rejects_boolean_in_zscore(cliente: TestClient, modelo: ModeloRiesgo) -> None:
    if not modelo.features_extra:
        pytest.skip("La variante no tiene z-scores.")
    campo = modelo.features_extra[0]
    registro = registro_demo(modelo)
    registro[campo] = True
    ...
    assert response.status_code == 422
    assert response.json()["detail"] == {"numero_invalido": campo}
...
def test_predict_batch_matches_model_and_sorts_risk(
    ...
    expected = [{**fila, "variante": modelo.variante} for fila in expected]
    response = cliente.post(
...
def test_predict_batch_rejects_more_than_5000(
    cliente: TestClient, modelo: ModeloRiesgo
) -> None:
    registro = registro_demo(modelo)
    response = cliente.post(
        "/predict/batch", json={"registros": [registro] * 5001}
    )

    assert response.status_code == 422


def test_predict_batch_rejects_zero_capacity(
    cliente: TestClient, modelo: ModeloRiesgo
) -> None:
    response = cliente.post(
        "/predict/batch?capacidad=0",
...
def test_predict_batch_rejects_invalid_second_record(
    cliente: TestClient, modelo: ModeloRiesgo
) -> None:
    primero = registro_demo(modelo)
    segundo = registro_demo(modelo)
    segundo.pop(modelo.features[-1])

    response = cliente.post(
        "/predict/batch", json={"registros": [primero, segundo]}
    )
...
def test_model_variants_lists_horizons_and_variants(cliente: TestClient) -> None:
    response = cliente.get("/model/variants")

    assert response.status_code == 200
    assert response.json() == modelos_disponibles()
...
def test_model_info_reports_missing_variant(cliente: TestClient) -> None:
    response = cliente.get("/model/info?variante=inexistente")
